...
class DynamicObstacle(BehaviorScript):
    """
    Class representing a moving obstacle. When attached to a moving obstacle, the script publishes the objects current
    and future locations to the GlobalCharacterPositionManager, allowing characters to avoid it.

    The script should be attached to small and slow moving objects like robots.

    FYI: Each 'on_init', 'on_destroy', 'on_play', 'on_pause', 'on_stop' and
    'on_update' are called by the Isaac Sim runtime.
    Refer to the omni.kit.scripting.__init__.py scritp for more information.
    """

    def on_init(self):
        carb.log_info(f"DynamicObstacle.on_init()->{self.prim_path}")
    ...
    def publish_object_position(self, delta_time):
        t = USDUtil.get_prim_pos(prim=self.prim_obj)
        n = 10
        ...
        self.character_manager.set_character_future_pos(
            self.object_path, CarbUtil.add3(t, CarbUtil.scale3(self.velocity_vec, radius / 0.5))
        )
        self.character_manager.set_character_radius(self.object_path, radius * 1.5)
