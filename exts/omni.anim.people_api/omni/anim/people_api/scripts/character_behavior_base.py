...
from omni.anim.people_api.scripts.custom_command.defines import CustomCommandTemplate
...
from omni.anim.people_api.scripts.seed_manager import CharacterSeedRegistry
...
class CharacterBehaviorBase(BehaviorScript, ABC):
    """
    Character controller class that reads commands from a command file and
    ...
    def __init__(self, prim_path: Sdf.Path):
        ...
        # Get character name from prim path
        character_name = str(prim_path).split("/")[-1]

        # Try to get seed from registry first, then use provided seed, then random
        registry = CharacterSeedRegistry.get_instance()
        registry_seed = registry.get_seed(character_name)

        if registry_seed is None:
            seed = random.randint(0, 2**32 - 1)
            carb.log_info(f"No seed provided for {character_name}, using random seed: {seed}")
        else:
            seed = registry_seed
    ...
    def on_init(self):
        """
        Called when a script is attached to characters and
        when a stage is loaded.
        Uses renew_character_state() to initialize character state.
        """
    ...
    def on_play(self):
        """
        Called when entering runtime (when clicking play button).
        Uses renew_character_state() to initialize character state.
        """
        self.renew_character_state()
    ...
    def on_stop(self):
        """
        Called when exiting runtime (when clicking stop button).
        Uses on_destroy() to clear state.
        """
        self.on_destroy()
    ...
    def renew_character_state(self):
        ...
        self.idle_duration_min = float(self.setting.get(PeopleSettings.IDLE_DURATION_MIN) or 1.0)
        self.idle_duration_max = float(self.setting.get(PeopleSettings.IDLE_DURATION_MAX) or 3.0)
        self.character_name = self.get_agent_name()
        carb.log_info(f"Character name is {self.character_name}")
        self.character = None
        ...
        # Expose the navmesh and the interface of it
    ...
    def end_current_command(self, set_status: bool = True):
        ...
        if self.current_command is not None:
            ...
            # if character is conducting "Queue" command,
            # remove next several commands related to this behavior.
            if self.current_command.get_command_name() == "QueueCmd" or self.in_queue:
                self.clean_unclosed_dequeue()

        # if character is currently inside the queue,
    ...
    # Remove the following queue behavior and "Dequeue" command
    # once the "Queue" command is interrupted by command injection
        ...
        if len(self.commands) > 1:
            ...
            for i in range(1, list_length):
                ...
                if str(self.commands[i][0]) == "Dequeue":
                    # remove all the command between
                    # this command and the second command.
                    if i == 1:
                        self.commands.pop(i)
                    else:
                        self.commands[1:i] = []
    ...
    def get_agent_name(self):
        ...
        # If a character is loaded through the spawn command,
        # the commands for the character can be given by
        # using the encompassing parent name.
    ...
    def register_to_agent_manager(self):
        ...
        carb.log_info(f"register to agent manager : -- {agent_name}")
    ...
    def subscription_to_command_start(self, current_command: Command):
        ...
        carb.eventdispatcher.get_eventdispatcher().dispatch_event(
            event_name=AgentEvent.CommandStartEvent,
            payload=command_info,
        )
        carb.log_info("create event: -- " f"command start with command info: {str(command_info)}")
    ...
    def subscription_to_command_end(self, current_command: Command, status: str | None = None):
        ...
        carb.log_info("create event: -- command end with command " f"info: {str(command_info)}")
    ...
    # Removed following method in Isaac Sim 5.0.0
    #   ''read_commands_from_file', 'get_combined_user_commands'
    # Also in Isaac Sim 5.0.0, the 'read_commands_from_UI' is removed

    # Convert command string to command list.
    # Split character name, command name, and command parameters
    def convert_str_to_command(self, cmd_line):
        if not cmd_line:
            return None

        words = str(cmd_line).strip().split(" ")
        # Check if the command is for this character
        # If the words start with the character name,
        # take the rest of the words as the command
        if words[0] == self.character_name:
            command = []
            command = [str(word) for word in words[1:] if word != ""]
            return command

        # Check if the command is for creating a queue
        # If the words start with "Queue", create a queue
        if words[0] == "Queue":
            self.queue_manager.create_queue(words[1])
            return None

        # Check if the command is for creating a queue spot
        # If the words start with "Queue_Spot", create a queue spot
        ...
        # If the words start with "#", it is a comment and should be ignored
        if words[0][0] == "#":
            return None

        return None

    # Moved this method to the subclasses.
    # (character_behavior_random_goto.py, character_behavior_random_idle.py)
    @abstractmethod
    def get_simulation_commands(self):
        """
        Abstract method that must be implemented by subclasses to define
        the simulation commands for the character.

        Returns:
            List: A list of command tuples for the character to execute.
        """
        pass
    ...
    def inject_command(
        ...
        """
        Inject commands into the current command queue.

        Args:
            command_list: A list of command info to inject.
            executeImmediately: Whether the commands should be executed
                immediately or appended to the end of the simulation.
            on_finished: A tuple containing callback info when injected
                commands finish execution.
                - The first value is the callback ID (str).
                - The second value is the callback function, invoked with
        ...
        carb.log_warn("After command injection, " f"commands for {self.character_name} are: {self.commands}")

    # Replace all commands in a character's command list
    def replace_command(self, command_list, on_finished: Tuple[str, Callable[[str, str], None]] = None):
        """
        Replace the current command list with the given one.

        Args:
            command_list: A list of commands to assign.
            on_finished: A tuple containing callback info when the
                commands finish execution.
                - The first value is the callback ID (str).
                - The second value is the callback function, invoked
                  with (callback_id, character_name).
        """
        cmd_array = self.handle_command_list(command_list)
        ...
        carb.log_warn("After command replacement, " f"commands for {self.character_name} are: {self.commands}")
    ...
    def handle_command_list(self, command_list):
        ...
        for command in command_list:
            ...
            if Utils.check_command_type(command) == "string":
                ...
                carb.log_warn("Error as warn message: " f"{command} has a wrong type : {type(command)}")
                continue
    ...
    def get_command(self, command_pair: Tuple[str, List[str]]) -> Optional[object]:
        """
        Return an instance of a command object based on the input.

        Args:
            command_pair (list[str]): List of strings describing the command.

        Returns:
            object: The instantiated command object.
        """
        ...
        if command[0] == "GoTo":
            ...
            module_str = f".commands.{command[0].lower()}"
            try:
    ...
    def execute_command(self, commands, delta_time):
        """
        Executes commands in commands list in sequence.
        Removes a command once completed.
        ...
        try:
            ...
            carb.log_error(
                f"{self.get_origin_command_string(self.current_command.command)}: "
                "invalid command. Abort this execution."
            )
    ...
    def on_update(self, current_time: float, delta_time: float):
        ...
        try:
            ...
            if not self._update_error_logged:
                carb.log_error(
                    f"CharacterBehaviorBase update failed for {self.prim_path}:\n{traceback.format_exc()}"
                )
