...
from omni.metropolis.utils.carb_util import CarbUtil, CarbSettingUtil
...
class NavigationManager:
    """
    Manages navigation for a character, such as generating static and dynamic obstacle free paths and
    publishing current and future positions of characters.
    """

    def __init__(self, character_name, navmesh_enabled, dynamic_avoidance_enabled=True, character=None):
        self.navmesh = nav.acquire_interface().get_navmesh()
        self.character_manager = GlobalCharacterPositionManager.get_instance()
        self.character_name = character_name
        self.character = character or ag.get_character(self.character_name)
    ...
    def calculate_rotation_diff(self):
        ...
        return MathUtil.get_rotation_angle_difference(char_rot_angle, target_rot_angle)
    ...
    def get_avoid_angle(self, radius_a, radius_b, collision_dist, movement_vector):
        movement_vector_length = CarbUtil.length3(movement_vector)
        if movement_vector_length == 0.0:
            return 0.0
        dist_to_avoid = radius_a + radius_b - CarbUtil.length3(collision_dist)
    ...
    def check_proximity_to_point(self, point, proximity_dist):
        ...
        return CarbUtil.dist3(char_pos, point_on_character_plane) < proximity_dist
    ...
    def publish_character_positions(self, delta_time, radius):
        ...
        self.velocity_vec = CarbUtil.scale3(
            CarbUtil.sub3(self.positions_over_time[-1], self.positions_over_time[0]), 1 / sum(self.delta_time_list)
        )
        ...
        self.character_manager.set_character_future_pos(
            self.character_name, CarbUtil.add3(char_pos, CarbUtil.scale3(self.velocity_vec, 1))
        )
    ...
    def update_target_path_progress(self):
        if len(self.path_targets) == 1:
            final_distance = CarbSettingUtil.get_value_by_key(PeopleSettings.CHARACTER_FINAL_TARGET_DISTANCE, 0.25)
            if self.check_proximity_to_point(self.path_targets[0], final_distance):
    ...
    def generate_goto_path(self, coords):
        if len(coords) < 4 or len(coords) % 3 != 1:
            raise ValueError(
                "Invalid coordinate list for path generation. Coordinate list must be a sequence of x,y,z with the"
                "last coordinate also specifying the ending rotation."
            )
    ...
    def detect_collision(self):
        ...
        for obstacle in self.character_manager.get_all_managed_characters():
            if obstacle != self.character_name:
                dist_between_future_pos = CarbUtil.dist3(
                ...
                dist_between_curr_pos = CarbUtil.dist3(
                ...
                if dist_between_future_pos < radius_sum:
                    self_movement_vector = CarbUtil.sub3(
                        self.character_manager.get_character_future_pos(self.character_name),
                        self.character_manager.get_character_current_pos(self.character_name),
                    )
                    obstacle_movement_vector = CarbUtil.sub3(
                        self.character_manager.get_character_future_pos(obstacle),
                        self.character_manager.get_character_current_pos(obstacle),
                    )
                    predicted_pos_difference = CarbUtil.sub3(
                        self.character_manager.get_character_future_pos(obstacle),
                        self.character_manager.get_character_future_pos(self.character_name),
                    )

                    # Check whether self_movement_vector and predicted_pos_difference are on the same side of the
                    # axis, if it is, add the obstacle to the collision list
                    # Check whether self_movement_vector and obstacle_movement_vector are on different side of the
                    # axis, if so, add the obstacle to collision list
                    # Check whether if the other obstacle's speed is too slow, if so add it collision list regardless
                    # of its moving direction.
                    if (
                        CarbUtil.dot3(self_movement_vector, predicted_pos_difference) > 0
                        or CarbUtil.dot3(self_movement_vector, obstacle_movement_vector) < 0
                        or CarbUtil.length3(obstacle_movement_vector) < radius_sum * 0.5
                    ):
    ...
    def update_path(self):
        ...
        if self.detect_collision():
            ...
            self_movement_vector = CarbUtil.normalize3(
                CarbUtil.sub3(
                    self.character_manager.get_character_future_pos(self.character_name),
                    self.character_manager.get_character_current_pos(self.character_name),
                )
            )
            self_movement_vector_right = CarbUtil.normalize3(
                CarbUtil.cross3(self_movement_vector, carb.Float3(0, 0, 1))
            )

            if len(self.collision_list) > 0:

                # If your destination is closer than collision, ignore the collision.
                if CarbUtil.length3(CarbUtil.sub3(current_pos, self.path_targets[-1])) < CarbUtil.length3(
                    CarbUtil.sub3(current_pos, self.character_manager.get_character_future_pos(self.collision_list[0]))
                ):
                    return
                ...
                angle = Gf.Clamp(
                    (radius_b / radius_a)
                    * self.get_avoid_angle(radius_a, radius_b, predicted_pos_difference, self_movement_vector),
                    0,
                    60,
                )
                left_future_point = CarbUtil.add3(
                    current_pos,
                    Utils.rotZ3(
                        CarbUtil.sub3(
                            self.character_manager.get_character_future_pos(self.character_name), current_pos
                        ),
                        angle,
                    ),
                )
                right_future_point = CarbUtil.add3(
                    current_pos,
                    Utils.rotZ3(
                        CarbUtil.sub3(
                            self.character_manager.get_character_future_pos(self.character_name), current_pos
                ...
                direction_of_collision = CarbUtil.dot3(
                    self_movement_vector_right, CarbUtil.normalize3(predicted_pos_difference)
                )

                # If no navmesh, pick the best avoidance point
                if not self.navmesh or not self.navmesh_enabled:
                    # if the value is between 0.2 and -0.2 it is ambiguous whether to move right or left.
                    # Ignore till collision direction is more deterministic.
                    ...
                    # If navmesh and the other object will avoid you, pick the best avoidance point if it is on the
                    # navmesh, if it is not then skip as the other object will avoid you anyway.
                    if Utils.is_character(self.collision_list[0]) and self.is_still_moving(self.collision_list[0]):
                        if direction_of_collision > 0.2 and SimulationUtil.validate_navmesh_point_2d(
                            [left_future_point.x, left_future_point.y, 0], agent_radius=0.5
                        ):
                            new_position = left_future_point
                        elif direction_of_collision <= -0.2 and SimulationUtil.validate_navmesh_point_2d(
                            [right_future_point.x, right_future_point.y, 0], agent_radius=0.5
                        ):
                            new_position = right_future_point
                        else:
                            return
                    # If the other object is not going to avoid you, then pick the best avoidance point as long as it
                    # is on the navmesh
                    else:
                        if SimulationUtil.validate_navmesh_point_2d(
                            [left_future_point.x, left_future_point.y, 0], agent_radius=0.5
                        ) and (
                            direction_of_collision > 0.2
                            or not SimulationUtil.validate_navmesh_point_2d(
                                [right_future_point.x, right_future_point.y, 0], agent_radius=0.5
                            )
                        ):
                            new_position = left_future_point
                        elif SimulationUtil.validate_navmesh_point_2d(
                            [right_future_point.x, right_future_point.y, 0], agent_radius=0.5
                        ) and (
                            direction_of_collision < -0.2
                            or not SimulationUtil.validate_navmesh_point_2d(
